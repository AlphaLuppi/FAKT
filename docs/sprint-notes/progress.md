...
- [x] **Track α** — scaffold `packages/api-server/` + middlewares (auth/zod/error-handler/request-id) + schemas (common/workspace/clients/settings) + 14 endpoints (health, workspace GET/POST/PATCH, settings GET/:key, PUT/:key, clients CRUD + restore + search), helper `createTestApp()` réutilisable par β/γ, 38 tests verts couvrant 401/400/404/409.
- [ ] Track δ — Tauri sidecar spawn + port discovery
...
- [ ] Track η — signature fixes
- [ ] Track θ — migration payment_notes + docs 3 modes
