...
function setupApis(options: { quotes: Quote[]; invoices: Invoice[] }): void {
  const qApi: Partial<QuotesApi> = {
    async list() { return options.quotes; },
    async get(id) { return options.quotes.find((q) => q.id === id) ?? null; },
  };
  const iApi: Partial<InvoiceApi> = {
    async list() { return options.invoices; },
    async get(id) { return options.invoices.find((i) => i.id === id) ?? null; },
  };
...
describe("DashboardRoute", () => {
  ...
  it("widget factures en retard : affiche le count overdue", async () => {
    ...
    setupApis({
      quotes: [],
      invoices: [
        invoice({ id: "i-1", status: "sent", dueDate: pastDue, totalHtCents: 300000, number: "F2026-001" }),
        invoice({ id: "i-2", status: "sent", dueDate: pastDue, totalHtCents: 200000, number: "F2026-002" }),
        invoice({ id: "i-3", status: "sent", dueDate: futureDue, totalHtCents: 100000, number: "F2026-003" }),
        invoice({ id: "i-4", status: "paid", dueDate: pastDue, totalHtCents: 500000, number: "F2026-004" }),
      ],
    });
    renderDashboard();
    await waitFor(() => {
  ...
  it("widget activité récente : affiche les entrées triées par date DESC", async () => {
    setupApis({
      quotes: [
        quote({ id: "q-1", createdAt: NOW - 6 * ONE_DAY, updatedAt: NOW - 6 * ONE_DAY, number: "D2026-001" }),
        quote({ id: "q-2", status: "sent", issuedAt: NOW - 2 * ONE_DAY, createdAt: NOW - 5 * ONE_DAY, updatedAt: NOW - 2 * ONE_DAY, number: "D2026-002" }),
      ],
      invoices: [
        invoice({ id: "i-1", status: "paid", paidAt: NOW - 1 * ONE_DAY, createdAt: NOW - 3 * ONE_DAY, updatedAt: NOW - 1 * ONE_DAY, number: "F2026-001" }),
        invoice({ id: "i-2", status: "sent", issuedAt: NOW - 4 * ONE_DAY, createdAt: NOW - 4 * ONE_DAY, updatedAt: NOW - 4 * ONE_DAY, number: "F2026-002" }),
      ],
    });
    renderDashboard();
    await waitFor(() => {
